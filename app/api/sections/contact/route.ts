...
<div style="font-family: sans-serif; max-width: 640px; margin: auto; border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.1); border: 1px solid #e2e8f0; overflow: hidden;">
  ...
  <div style="padding: 32px;">
    ...
    <div style="margin-top: 28px; text-align: center;">
      <a href="mailto:${email}" style="background: #2563eb; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none;">Reply to ${name.split(" ")[0]}</a>
    </div>
  </div>
  <div style="background: #f8fafc; padding: 20px 32px; text-align: center;">
    ...
    await transporter.sendMail({
      from: `"Website Contact" <${SMTP_MAIL}>`,
      to: SMTP_MAIL,
      replyTo: email,
      text: emailText,
      html: emailHtml,
